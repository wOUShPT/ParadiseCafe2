...
public class DialogueTrigger : MonoBehaviour
{
    ...
    private void Update()
    {
        dialoguePrompt.SetActive(false);
        RaycastHit hit;
        if (Physics.SphereCast(sphereCastOrigin.position, 2, transform.forward, out hit, dialogueMaxDistance+2) && hit.transform.CompareTag(playerTag) && (hit.distance < dialogueMaxDistance && hit.distance > dialogueMinDistance))
        {
